...
from PySide6.QtWidgets import (
    QDialog, QPushButton, QMessageBox, QComboBox, 
    QHBoxLayout, QLabel, QVBoxLayout, QApplication
)
...
from .settings import TextSetting, DropdownSetting
...
class OllamaStateManager(QObject):
    ...
    def find_ollama_executable(self, force_refresh: bool = False) -> Optional[str]:
        ...
        """
        if not force_refresh and self._ollama_path and \
           self._is_cache_valid(self._path_check_time, self.CACHE_DURATION):
    ...
    def is_ollama_installed(self, force_refresh: bool = False) -> bool:
        ...
        """
        if not force_refresh and self._is_installed is not None and \
           self._is_cache_valid(self._path_check_time, self.CACHE_DURATION):
    ...
    def is_ollama_running(self, force_refresh: bool = False) -> bool:
        ...
        """
        if not force_refresh and self._is_running is not None and \
           self._is_cache_valid(self._running_check_time, self.QUICK_CHECK_DURATION):
    ...
    def get_ollama_models(self, force_refresh: bool = False) -> List[Tuple[str, str]]:
        ...
        """
        if not force_refresh and self._models_list and \
           self._is_cache_valid(self._models_check_time, self.CACHE_DURATION):
        ...
        try:
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=0.5,  # Slightly longer timeout for list operation
            )
...
class OllamaProvider(AIProvider):
    ...
    def _install_ollama_async(self):
        ...
        from .ollama_progress_window import OllamaInstallProgressWindow
    ...
    def _delete_model(self):
        ...
        warning_label = QLabel(
            "⚠️ Warning: This will permanently delete the selected model."
        )
    ...
    def _get_response_impl(
        ...
        **kwargs,
        ...
        """Send request to Ollama server - synchronous implementation."""
