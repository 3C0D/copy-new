...
from config.constants import DEFAULT_ACTIONS
...
class DraggableButton(QtWidgets.QPushButton):
    ...
    def dropEvent(self, event):
        ...
        source_idx = int(
            event.mimeData().data("application/x-button-index").data().decode()
        )
...
class CustomPopupWindow(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        if self.app.settings_manager.settings.custom_data:
            update_available = self.app.settings_manager.settings.custom_data.get(
                "update_available", False
            )
    ...
    def get_actions(self):
        ...
        if (
            ...
            logging.warning("Settings manager not available, using DEFAULT_ACTIONS")
    ...
    def on_reset_clicked(self):
        """
        Reset options to DEFAULT_ACTIONS and reload the interface.
        """
        confirm_box = QtWidgets.QMessageBox()
        confirm_box.setWindowFlags(
            confirm_box.windowFlags() | QtCore.Qt.WindowStaysOnTopHint
        )
        ...
        if confirm_box.exec_() == QtWidgets.QMessageBox.Yes:
            try:
                logging.debug("Resetting to default actions from DEFAULT_ACTIONS")
                ...
                success_msg.setWindowFlags(
                    success_msg.windowFlags() | QtCore.Qt.WindowStaysOnTopHint
                )
                ...
                logging.error(f"Error resetting options: {e}")
                self.app.show_message_signal.emit(
                    "Error", f"An error occurred while resetting: {str(e)}"
                )
    ...
    def delete_button_clicked(self, btn):
        ...
        confirm.setText(f"Are you sure you want to continue?")
        ...
        if confirm.exec_() == QtWidgets.QMessageBox.Yes:
            try:
                ...
                self.app.show_message_signal.emit(
                    "Error", f"An error occurred while deleting the button: {str(e)}"
                )
