...
class SettingsManager:
    ...
    def _resolve_config_directory(self) -> Path:
        ...
        if self._is_build_final():  # sourcery skip
    ...
    def _load_user_data(self) -> Optional[Dict[str, Any]]:
        """Load user data from the data file - Version avec type explicite."""
        try:
            with open(self.data_file, encoding="utf-8") as f:
                raw_data = json.load(f)

            # Validation optionnelle que c'est bien un dict
    ...
    def has_providers_configured(self) -> bool:
        ...
        active_provider = getattr(self.settings.system, "provider", None)
    ...
    def _get_providers(self) -> dict[str, ProviderConfig]:
        """Get providers configuration from custom data."""
        if not self.settings or not self.settings.custom_data:
            return {}
    ...
    def update_system_setting(self, key: str, value: Any) -> bool:
        ...
        # For TypedDict, check if key is valid by trying to access it
        try:
            # Check if key exists in the TypedDict annotations
            from .constants import DEFAULT_SYSTEM_VALUES

            if key in DEFAULT_SYSTEM_VALUES:
                self.settings.system[key] = value
                return self.save_settings()
            self._logger.error(f"Unknown system setting: {key}")
            return False
        except Exception as e:
            self._logger.error(f"Error updating system setting {key}: {e}")
            return False
