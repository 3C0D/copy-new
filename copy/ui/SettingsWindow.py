...
class SettingsWindow(QtWidgets.QWidget):
    ...
    def __init__(self, app, providers_only=False):
        ...
        self.previous_window = None  # Store reference to previous window
        self.init_ui()
        self.retranslate_ui()
    ...
    def init_provider_ui(self, provider: AIProvider, layout):
        ...
        if provider.logo:
            logo_path = os.path.join(
                os.path.dirname(sys.argv[0]), "icons", f"provider_{provider.logo}.png"
            )
        ...
        if hasattr(provider, "ollama_button_text"):
            ...
            if provider.button_text:
                ...
                self.current_provider_layout.addWidget(
                    button, alignment=QtCore.Qt.AlignmentFlag.AlignCenter
                )
        ...
        if (
            ...
            not in self.app.settings_manager.settings.custom_data["providers"]
        ):
            self.app.settings_manager.settings.custom_data["providers"][
                provider.provider_name
            ] = {}
        ...
        for setting in provider.settings:
            saved_value = self.app.settings_manager.settings.custom_data["providers"][
                provider.provider_name
            ].get(setting.name, setting.default_value)
    ...
    def init_ui(self):
        ...
        main_layout.setSpacing(
            10
        )  # Add spacing between scroll area and bottom elements
        ...
        self.provider_dropdown.wheelEvent = lambda event: event.ignore()

        current_provider = self.app.settings_manager.settings.system.provider
